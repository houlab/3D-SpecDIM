...
if length(varargin)>1
  for n = 1:2:length(varargin)
    switch(lower(varargin{n}))
        ...
        case 'delta'
            model.delta = varargin{n+1};
        case 'frame_id'
            model.frame_id = varargin{n+1};
    end
...
if isempty(model.xdata)
    model.xdata = [0 siz(2)]; %5
end
if isempty(model.ydata)
    model.ydata = [0 siz(1)]; %5
end
if isempty(model.zdata)
    model.zdata = [0 siz(3)]; %11
end
...
h = findobj(ax,'type','slice','tag',model.tag);
% for n = 1:length(h)
%   try
%      delete(h(n));
%   catch
...
handle_ind = 1 + (model.frame_id - 1) * siz(3);

% Create z-slice(Ê¦ÐÖ°æ)
if(ind==3 || is3DTexture )    
  ...
  y_max = max(y(:));
  z_min = min(z(:));
  z_max = z_min + delta*size(cdata,3)-1;
  [X,Y,Z] = meshgrid(x_min:x_max-1, y_min:y_max-1, z_min:delta:z_max);
  h1=slice(ax, X,Y,Z,cdata,[x_min:x_max-1],[y_min:y_max-1],[z_min:delta:z_max]);
  n_cv=5+5+11;
  for i = 1:n_cv
      set(h1(i),'EdgeColor','none','FaceColor','interp',...
  ...
  for n = 1:size(cdata,3)
    ...
    aslice = (aslice-min(min(aslice)))/(max(max(aslice))-min(min(aslice)));
    m = max(max(aslice));
    aslice(find(aslice<m*0.7)) = aslice(find(aslice<m*0.7)).^2;
...
% % Create z-slice(Ô­°æ)
% if(ind==3 || is3DTexture )    
%   x = [model.xdata(1), model.xdata(2); model.xdata(1), model.xdata(2)] + model.delta(1);
%   y = [model.ydata(1), model.ydata(1); model.ydata(2), model.ydata(2)] + model.delta(2);
%   z = [model.zdata(1), model.zdata(1); model.zdata(1), model.zdata(1)] + model.delta(3);
%   diff = model.zdata(2)-model.zdata(1);
%   delta = diff/size(cdata,3);
%   for n = 1:size(cdata,3)
% 
