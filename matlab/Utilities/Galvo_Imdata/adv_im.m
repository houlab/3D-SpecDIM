classdef adv_im < adv_im_tl
    ...
    properties
        ...
        bin_time = 28; % bin time in us (eod cycle time)
        ...
        times_fit_interval = 1e-6; % 区分轨迹坐标的间隔时间
    ...
    methods
        ...
        function tmp = to_mat(self)
            ...
            tmp(:,9) = uint32(self.get_t()./self.TICK);
        ...
        function [data,data_interval] = to_mat_v2(self)
            % 
            ...
            data(:,6) = data(:,6).*self.x_conv;
            data(:,7) = data(:,7).*self.y_conv;
            data(:,8) = data(:,8).*self.z_conv;
            data(:, 12) = t;

            % 此函数用于提取特定间隔时间时的粒子轨迹坐标
            % 单位 s
            % 若 TAG的值为空，则该点为假光子点应当舍去
            data(isnan(data(:,5)),:) = [];
            % 生成查询的时间点序列
            times_query = (data(1,12):self.times_fit_interval:data(end,12))';
            % 使用 interp1 快速找到最接近的索引
            positions = interp1(data(:,12), 1:numel(data(:,12)), times_query, 'previous');
            data_interval = data(positions,6:8);
            data_interval(:,4) = times_query;
